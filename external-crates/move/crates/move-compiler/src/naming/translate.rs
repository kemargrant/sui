...
fn match_arm(context: &mut Context, sp!(aloc, arm): E::MatchArm) -> N::MatchArm {
    ...
    for (pat_var, guard_var) in guard_binder_pairs {
        guard_binders.add(pat_var, guard_var);
    }

    // Finally we handle the pattern, replacing unused variables with wildcards
    let pattern = *pat(context, Box::new(pattern));
    ...
    let pat_: N::MatchPattern_ = match pat_ {
                ...
                context.resolve_variant_name(ploc, "pattern", name, etys_opt)
            {
                ...
                let args = UniqueMap::maybe_from_iter(args.value.into_iter().enumerate().map(
                    |(idx, p)| {
                        ...
                        (field, (idx, *pat(context, Box::new(p))))
                    },
                ))
                .unwrap();
        ...
        EP::FieldConstructor(name, etys_opt, args) => {
                ...
                context.resolve_variant_name(ploc, "pattern", name, etys_opt)
            {
                ...
                let args = args.map(|_, (idx, p)| (idx, *pat(context, Box::new(p))));
                NP::Constructor(mident, enum_, variant, tys_opt, args)
            } else {
                assert!(context.env.has_errors());
                NP::ErrorPat
        ...
        EP::Or(lhs, rhs) => NP::Or(pat(context, lhs), pat(context, rhs)),
        EP::At(binder, body) => {
            if let Some(binder) = context.resolve_pattern_binder(binder.loc(), binder.0) {
                NP::At(binder, pat(context, body))
            } else {
                assert!(context.env.has_errors());
                pat(context, body).value
            }
        }
    };
